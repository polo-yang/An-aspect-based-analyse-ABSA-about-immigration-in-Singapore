...
class Inferer:
    ...
    def evaluate(self, text, aspect):
        ...
        t_inputs = [torch.tensor(np.array([data[col]]), device=self.opt.device) for col in self.opt.inputs_cols]
...
if __name__ == '__main__':
    ...
    dataset_files = {
        'twitter': {
            'train': '/content/drive/MyDrive/ABSA/datasets/twitter/train.seg',
            'test': '/content/drive/MyDrive/ABSA/datasets/twitter/test.seg'
        },
        'restaurant': {
    ...
    opt.state_dict_path = '/content/drive/MyDrive/ABSA/state_dict/1'
    opt.embed_dim = 300
    opt.hidden_dim = 300
    opt.max_seq_len = 85
    opt.bert_dim = 768
    ...
    fname='/content/drive/MyDrive/ABSA/output.txt'
    fin = open(fname, 'r', encoding='utf-8', newline='\n', errors='ignore')
    lines = fin.readlines()
    fin.close()
    all_data = []
    
    f = open("/content/drive/MyDrive/ABSA/value-output.txt", "a")
    for i in range(0, len(lines), 3):
        ...
        print(text,file=f)
        print(aspect,file=f)
        print(t_probs.argmax(axis=-1) - 1,file=f)
    f.close()
